...
- **The structured half of `query_search_analytics` is budgeted.** The text block was held to the 100 kB result budget by halving the table; `structuredContent.rows` carried every row Google returned beside it — 25 000 rows with a page and a query dimension came to 47 MB in one result. The structured half now carries exactly the rows the table shows, measured as sent, with a `truncated` block naming the next `start_row`; `rowCount` is the number Google returned.
- **A boundary between Google's JSON and the promises made about it.** Every tool declares an output schema and the SDK holds the answer to it, so a value of the wrong shape was a whole call answered with `Output validation error` or a crash: `rows: {}` (`rows.slice is not a function`), a key that was a number (`value.replace is not a function`), `site: null` on a verification resource (`Cannot read properties of null` out of `list_verified_sites` and `setup_site`), an empty `200` (`Buffer.byteLength` on `undefined`, from `get_site`, `get_sitemap`, `get_indexing_status`, `request_indexing` and `update_site_owners`), and `inspectionResult: 0`. Rows, keys, metrics, site blocks and empty answers are now shaped at the boundary — an entry of the wrong shape is dropped or written out as text, never the reason the listing fails — and a property test feeds every read tool arbitrary JSON and shaped envelopes with arbitrary leaves through the whole server.
- **`get_indexing_status` has an open output schema.** It declared a closed object and answered with Google's record verbatim, so a field Google adds made a client that had listed the tools throw `ProtocolError: … must NOT have additional properties`. Open now, like every other pass-through record, with the three named fields held to their types at the boundary. The test harness lists the tools on every connection, which is what would have caught this.
- **The publish job installs with `--ignore-scripts`.** It holds the OIDC token for npm Trusted Publishing, and every dependency's install hook ran while it did; nothing in the tree needs one. `gh release create` now carries `--verify-tag`.
- **The status is decided before the body is read.** A `429` or a `5xx` whose body was larger than the response ceiling was refused for its size — an error naming no status — so the retry keyed on the status never ran and the hint for it was never shown. An error body is now read under its own 64 kB ceiling and cut, never refused; success bodies are read under the ceiling for their endpoint (1 MB for a record, 8 MB for a URL inspection, 64 MB for a search analytics query), where every endpoint used to share the 64 MB one.
- **A domain property is a hostname.** `sc-domain:` accepted anything without a slash, so `sc-domain:foo bar`, a line break followed by instructions, or `user@host:port` normalised cleanly and reached the sentence of a confirmation dialog — the one line a person acts on, and one that `unverify_site` builds from the identifier of a verification resource Google returned. The part after `sc-domain:` now has to be a hostname: no whitespace or control character, no credentials, port or path, labels of at most 63 characters.
- **Text is cleaned on the way out.** Control characters reached `structuredContent` raw (the text block only escaped them because it is JSON) and lone surrogates went through as `\ud800`. Every upstream result now has C0/C1 controls and DEL stripped, lone surrogates repaired, and `__proto__` keys kept as own properties, in both channels; table cells are cleaned before they are escaped, and every cut runs `toWellFormed()`.
- **Text quoted from an error body is labelled, stripped and cut**, as is a `content-type` header the answerer chose. The access token is checked before it becomes a header: the runtime's refusal of a header value quotes the whole value, and that refusal used to travel through the generic error path into the tool result.
- **Configuration diagnostics describe a rejected value instead of printing it.** `ELICITATION … got "<value>"`, and the `normalizeSiteUrl` messages reached from `GSC_SITE_URL`, `GSC_ALLOWED_SITES` and every `site_url` argument, echoed the value in full — a JWT, a JSON key or a client secret pasted into the line below the credential. A value is now quoted only when it is short and shaped like a setting or a property; a hundred-kilobyte `site_url` comes back as a sentence, not as itself. OAuth values and the key file path with a control character inside are refused at startup, naming the variable.
- **Caller strings have ceilings**: 2 048 characters for a property, a URL and a resource id, 254 for an owner address and 100 owners, 35 for a language tag (letters, digits and hyphens), 4 096 for a filter expression and 50 filters, 64 for a confirmation token. The request path is cut in error messages.
- **A batch call has a wall-clock budget.** `submit_sitemaps` at fifty entries, each retried up to three times with backoff, could hold one call for most of two hours during an outage. Both batch tools now stop after two minutes, checked before each request, and say how many entries were attempted.
- **The result shortener remembers where it cut, not what a value looks like.** It skipped every string that ended in its own note, `… (N more characters omitted)`, so a value the crawled site chose to end with those words was never shortened, the budget could not be met, and the tool answered an error for that one item; the dropped count of an array was read back out of its last element, which the backend could have written. Shortened slots are now remembered by identity for the duration of one pass. The pass also cuts several candidates per round, largest first, where one cut per round and a full serialisation after each made five megabytes of long strings cost eleven seconds on the thread that serves every request.
- **Credential redaction is linear and complete.** The PEM pattern's lazy match made a message with repeated `-----BEGIN` markers quadratic, and a key cut off before its `END` line was not redacted at all. The pattern now accepts the end of the text as the end of a key, and reads at most 16 kB.
- **Dependency review on every pull request**, pinned, failing on a high-severity advisory in the change.
- **The runtime image carries neither yarn, corepack nor the lockfile.** Only npm had been removed.
- **`SECURITY.md` argues from the code that runs.** It named a `tupleResourceKey` that no longer exists, said a sealed dialog answer presented twice was honoured twice, and reasoned from a protocol revision the server no longer speaks alone; mcp-approval has carried a nonce since 0.8.1 and `serveStdio` negotiates both eras.
