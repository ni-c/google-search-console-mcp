 ...
 * file and in the README table, and a paste into the wrong one is a mistake
 * people make. Echoing the value would then print the credential into the
 * client's log — the same reason `loadConfig` never echoes a rejected key.
 *
...
function describeEntry(entry: string): string {
  // The charset is what actually catches a pasted credential here: every secret
  // this server takes carries characters a tool name cannot have — a refresh
  // token has `/` and upper-case letters, a service account key is JSON with
  // braces and newlines, and a key file path has slashes and dots. The length
  // bound is the second line of defence, and it has to clear
  // `query_search_analytics` and `get_verification_token` at 22 — a shorter cap
  // would redact the longest real names instead.
...
export function buildToolFilter(config: {
  ...
  if (allow === undefined) {
    ...
    for (const entry of allow) {
      ...
      const matches = expand(entry, 'GSC_ALLOW_TOOLS');
      if (matches.length === 0) {
        throw new ToolFilterError(
          `GSC_ALLOW_TOOLS: no tool matches ${describeEntry(entry)}. ` +
      ...
      if (survivors.length === 0) {
        if (entry.endsWith('*')) {
          ...
          console.error(
            `google-search-console-mcp: GSC_ALLOW_TOOLS: ${describeEntry(entry)} matches only write ` +
              'tools, which GSC_READ_ONLY suppresses — it contributes nothing.'
          );
        ...
        throw new ToolFilterError(
          `GSC_ALLOW_TOOLS: ${describeEntry(entry)} is a write tool, but GSC_READ_ONLY ` +
            'is set — it is never registered. Remove it from GSC_ALLOW_TOOLS, ' +
            `or unset GSC_READ_ONLY. Available in read-only mode: ${[...READ_TOOLS].sort().join(', ')}.`
        );
      }
  ...
  for (const entry of deny ?? []) {
    ...
    const matches = expand(entry, 'GSC_DENY_TOOLS');
    if (matches.length === 0) {
      throw new ToolFilterError(
        `GSC_DENY_TOOLS: no tool matches ${describeEntry(entry)}. Valid tools: ${catalogueList()}.`
      );
    }
  ...
  if (selected.size === 0) {
    throw new ToolFilterError(
      suppressedByReadOnly
        ? 'GSC_ALLOW_TOOLS selects only write tools, but GSC_READ_ONLY is ' +
            'set — the server would start with an empty tool list.'
        : 'GSC_ALLOW_TOOLS/GSC_DENY_TOOLS leave no tools registered — the ' +
